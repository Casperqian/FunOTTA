...
if __name__ == "__main__":
    ...
    if args.dataset in vars(datasets):
        dataset = vars(datasets)[args.dataset](args.data_dir, args.train_envs)
    else:
    ...
    for env_i, env in enumerate(dataset):
        if env_i not in args.train_envs:
            continue
    ...
    algorithm = algorithm_class(dataset.input_shape, dataset.num_classes,
                                len(args.train_envs), dataset.steps, hparams)
    ...
    def save_checkpoint(filename):
        ...
        save_dict = {
            ...
            "model_num_domains": len(args.train_envs),
    ...
    for step in tqdm(range(start_step, n_steps)):
        ...
        if ((step % checkpoint_freq == 0) or
            (step == n_steps - 1)) and step != 0:
            results = {
                'step': step,
            ...
            for name, loader, weights in evals:
                acc, auc, f1 = misc.evaluate(algorithm, loader, device)
            ...
            records = Q(records)
            scores = records.map(
                model_selection.IIDAUCSelectionMethod._step_auc)
            if scores[-1] == scores.argmax('val_auc'):
