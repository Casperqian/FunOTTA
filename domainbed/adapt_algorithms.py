...
class AttentionAugLayer(nn.Module):

    def __init__(self, dim, head_dim, num_heads, dropout=0.):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = head_dim
        self.dropout = nn.Dropout(dropout)

        self.embd_layers = nn.Linear(dim, self.num_heads * self.head_dim)
        self.proj = nn.Linear(self.head_dim * self.num_heads, dim)
        self.ffn = nn.Linear(dim, dim)

    def forward(self, fx_in, fp_in):
        fp_in = fp_in.squeeze(0)
        d = math.sqrt(self.head_dim)

        Nx = fx_in.size(0)
        f = torch.cat([fx_in, fp_in], dim=0)  # Nx + Np (Nx is batch size)

        f = self.embd_layers(f).view(-1, self.num_heads,
                                     self.head_dim).transpose(0, 1)

        fx, fp = f[:, :Nx], f[:, Nx:]

        w = F.softmax(torch.matmul(fx, fp.transpose(-2, -1)) / d,
                      dim=-1)  # head x Nx x Np
        w = self.dropout(w)

        fx = torch.matmul(w, fp)  # head x Nx x head_dim

        fx = fx.transpose(0, 1).contiguous().view(Nx, -1)
        fx = self.proj(fx)
        # fx = self.ffn(F.relu(fx))

        return fx


class AttentionAug(nn.Module):

    def __init__(self, dim, head_dim, init_mode, dropout=0., num_layers=2):
        super().__init__()
        ...
        self.layers = nn.ModuleList([
            AttentionAugLayer(dim, head_dim, self.num_heads, dropout)
    ...
    def forward(self, fx_in, fp_in):
        for layer in self.layers:
            fx_in = layer(fx_in, fp_in)
        return fx_in

    def reset(self):
        init_details = [0, 1]
        for layer in self.layers:
            initialize_tensor(layer.embd_layers.weight, self.init_mode,
                              init_details)
            if layer.embd_layers.bias is not None:
                initialize_tensor(layer.embd_layers.bias, "zeros")

            initialize_tensor(layer.proj.weight, self.init_mode, init_details)
            if layer.proj.bias is not None:
                initialize_tensor(layer.proj.bias, "zeros")

            initialize_tensor(layer.ffn.weight, self.init_mode, init_details)
            if layer.ffn.bias is not None:
                initialize_tensor(layer.ffn.bias, "zeros")
